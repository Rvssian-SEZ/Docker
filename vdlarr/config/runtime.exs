...
#
#     PHX_SERVER=true bin/vdlarr start
#
# Alternatively, you can use `mix phx.gen.release` to generate a `bin/server`
# script that automatically sets the env var above.
if System.get_env("PHX_SERVER") do
  config :vdlarr, VdlarrWeb.Endpoint, server: true
end

config :vdlarr,
  basic_auth_username: System.get_env("BASIC_AUTH_USERNAME"),
  basic_auth_password: System.get_env("BASIC_AUTH_PASSWORD"),
  login_username: System.get_env("LOGIN_USERNAME"),
  login_password: System.get_env("LOGIN_PASSWORD")
...
config :vdlarr, Vdlarr.Repo,
  load_extensions: [
    Path.join([:code.priv_dir(:vdlarr), "repo", "extensions", "sqlean-linux-#{system_arch}", "sqlean"])
  ]
...
if config_env() == :prod do
  ...
  db_path = System.get_env("DATABASE_PATH", Path.join([config_path, "db", "vdlarr.db"]))
  log_path = System.get_env("LOG_PATH", Path.join([config_path, "logs", "vdlarr.log"]))
  ...
  yt_dlp_plugin_directory = System.get_env("YT_DLP_PLUGIN_DIRECTORY", "/etc/yt-dlp/plugins")
  tmpfile_path = System.get_env("TMPFILE_PATH", Path.join([System.tmp_dir!(), "vdlarr", "data"]))
  ...
  config :vdlarr,
    ...
    yt_dlp_plugin_directory: yt_dlp_plugin_directory,
  ...
  config :vdlarr, Vdlarr.PromEx, disabled: !enable_prometheus
