defmodule VdlarrWeb.CustomComponents.TableComponents do
  @moduledoc false
  ...
  def table(assigns) do
    ~H"""
    <table class={["w-full table-auto bg-midnight-850", @table_class]}>
      <thead>
        <tr class="text-left border-b border-white/5">
          <th
            :for={col <- @col}
            class={[
              "px-4 py-4 font-medium text-[10px] uppercase tracking-wider text-slate-500",
      ...
      <tbody>
        <tr :for={row <- @rows} class="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
          <td
            :for={col <- @col}
            class={[
              "px-4 py-5",
