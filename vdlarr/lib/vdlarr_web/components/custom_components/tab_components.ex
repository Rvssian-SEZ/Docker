defmodule VdlarrWeb.CustomComponents.TabComponents do
  @moduledoc false
  ...
  def tabbed_layout(assigns) do
    ...
    <div
      x-data={"{
        ...
        activeClasses: 'text-indigo-400 border-indigo-400',
        inactiveClasses: 'border-transparent text-slate-400'
      }"}
      @hashchange.window={"openTab = getTabFromHash(openTab, '#{@first_tab_id}')"}
      class="w-full"
    >
      <header class="flex flex-col md:flex-row md:justify-between border-b border-white/5">
        <div class="flex flex-wrap gap-5 sm:gap-10">
          <a
            ...
            class="border-b-2 py-4 w-full sm:w-fit text-sm font-medium hover:text-white transition-colors md:text-base"
          >
