defmodule Pinchflat.Downloading.DownloadProgress do
  @moduledoc """
  Parses yt-dlp's `--progress-template` output lines (emitted while a media
  item is downloading - see `Pinchflat.Downloading.DownloadOptionBuilder.default_options/1`)
  ...
  `Pinchflat.Utils.CliUtils.wrap_cmd/4`'s `:line_handler` option). Lines that
  ...
  defp broadcast_progress(media_item_id, json) do
    case Phoenix.json_library().decode(json) do
      {:ok, progress} ->
        PinchflatWeb.Endpoint.broadcast("downloads:progress", "progress", %{
