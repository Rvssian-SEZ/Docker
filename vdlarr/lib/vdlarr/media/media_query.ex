defmodule Vdlarr.Media.MediaQuery do
  @moduledoc """
  ...
  # all in one go
  # usage: use Vdlarr.Media.MediaQuery
  ...
  # Deliberately overlaps with pending/0: a non-retryable error (see
  # MediaDownloadWorker.non_retryable_error?/1) never sets prevent_download, so an item
  # can be both "pending" (and thus silently re-attempted forever) and "failed" at the
  # same time. Surfacing it here regardless of its pending status is the point - see
  # the Failed Downloads tab. `not downloaded()` is required (not just belt-and-suspenders):
  # a {:recovered, ...} download can set last_error on an item that DID successfully
  # download, and that item should never show up as "failed".
  def failed do
    dynamic([mi], ^has_error() and not (^downloaded()) and not (^download_prevented()))
  end
