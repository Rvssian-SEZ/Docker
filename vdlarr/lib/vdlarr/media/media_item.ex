defmodule Vdlarr.Media.MediaItem do
  @moduledoc """
  ...
  alias Vdlarr.Media.MediaItemsSearchIndex
  ...
  schema "media_items" do
    ...
    # yt-dlp's own extractor name (e.g. "youtube", "vimeo"). Used as the prefix in
    # `--download-archive` dedup files. May be nil for records created before this
    # field existed - callers should fall back to "youtube" in that case.
  ...
  defp do_update_upload_date_index(%{changes: changes} = changeset) when is_map_key(changes, :uploaded_at) do
    ...
    # and playlists append new videos to the bottom of the list. `:video` sources only
    # ever have a single media item, so the index/aggregator/direction don't really
    # matter, but we spell it out explicitly (matching the playlist behaviour) rather
    # than relying on incidental fallthrough.
    {default_index, aggregator, change_direction} =
