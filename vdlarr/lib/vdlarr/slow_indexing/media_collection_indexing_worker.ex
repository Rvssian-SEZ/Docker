defmodule Vdlarr.SlowIndexing.MediaCollectionIndexingWorker do
  @moduledoc false
  ...
  alias Vdlarr.Lifecycle.Notifications.SourceNotifications
  ...
  - If this drift is undesirable for a given source, setting a cron schedule
    (`Source.index_cron_schedule`, see `Source.cron_scheduled?/1`) instead runs
    the job at fixed wall-clock times computed from the cron expression, so it
    never drifts - at the cost of the overlap/rate-limit protection above.
  ...
  Order of operations:
    ...
    5. If the job reschedules, the cycle from step 3 repeats until the heat death
  ...
  def perform(%Oban.Job{args: %{"id" => source_id} = args}) do
    ...
    cond do
      Source.cron_scheduled?(source) or source.index_frequency_minutes > 0 ->
        # If the indexing is on a schedule (cron or interval) simply run indexing and reschedule
        perform_indexing_and_notification(source, was_forced: args["force"])
        reschedule_indexing(source)

      is_nil(source.last_indexed_at) ->
  ...
  defp reschedule_indexing(source) do
    scheduling_opts = SlowIndexingHelpers.scheduling_opts_for(source)

    %{id: source.id}
    |> MediaCollectionIndexingWorker.new(scheduling_opts)
    |> Tasks.create_job_with_task(source)
    |> case do
      {:ok, task} -> {:ok, task}
