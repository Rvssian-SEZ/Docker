defmodule Vdlarr.Boot.PreJobStartupTasks do
  @moduledoc """
  ...
  alias Vdlarr.Lifecycle.UserScripts.CommandRunner, as: UserScriptRunner

  def start_link(opts \\ []) do
    GenServer.start_link(__MODULE__, %{env: Application.get_env(:vdlarr, :env)}, opts)
  end
  ...
  # `check_and_update_timezone/0` (in Vdlarr.Application) validates the TZ/TIMEZONE
  # env var and seeds `Application.env` with it, but it runs before the Repo exists so
  # it has no idea about the DB-backed setting. Reconcile the two here, every boot:
  #   - no persisted value yet -> seed the DB from the env-var-validated boot value
  #   - a persisted value exists -> it's authoritative, re-apply it into Application.env
  #     so a DB-persisted change survives a restart without needing the env var updated
  defp apply_timezone_setting do
    case Settings.get!(:timezone) do
      nil -> Settings.set(timezone: Application.get_env(:vdlarr, :timezone))
      tz -> Application.put_env(:vdlarr, :timezone, tz)
    end
  end

  defp run_app_init_script do
    runner = Application.get_env(:vdlarr, :user_script_runner, UserScriptRunner)

    runner.run(:app_init, %{})
  end

  defp yt_dlp_runner do
    Application.get_env(:vdlarr, :yt_dlp_runner)
  end

  defp apprise_runner do
    Application.get_env(:vdlarr, :apprise_runner)
  end
end
