defmodule Vdlarr.Settings.YtDlpConfigFile do
  @moduledoc """
  ...
  `Vdlarr.Boot.PreJobStartupTasks`. `DownloadOptionBuilder.config_file_options/1`
  already reads it (along with the per-source/media-profile/media-item tiers at
  the same path) when building yt-dlp's download options - this module is just
  the read/write side so it can be edited from the Settings page instead of by
  ...
  """
  def filepath do
    Path.join(Application.get_env(:vdlarr, :extras_directory), @relative_path)
  end
