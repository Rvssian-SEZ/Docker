defmodule Vdlarr.YtDlp.Media do
  @moduledoc """
  ...
  def indexing_output_template do
    "%(.{id,title,live_status,original_url,description,aspect_ratio,duration,upload_date,timestamp,playlist_index,filename,extractor})j"
  end
  ...
  def response_to_struct(response) do
    %Media{
      ...
      # yt-dlp's own extractor name (e.g. "youtube", "vimeo") - used verbatim as the
      # prefix in `--download-archive` files. Falls back to "youtube" downstream for
      # any pre-existing records that predate this field.
