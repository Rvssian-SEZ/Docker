defmodule Vdlarr.Downloading.MediaDownloader do
  @moduledoc """
  ...
  alias Vdlarr.Metadata.NfoBuilder
  alias Vdlarr.Metadata.MetadataParser
  ...
  def download_for_media_item(%MediaItem{} = media_item, override_opts \\ []) do
    case attempt_download_and_update_for_media_item(media_item, override_opts) do
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        # The download itself succeeded, but saving the resulting attrs (eg: metadata) failed
        # validation - most commonly a failed thumbnail fetch (see MediaMetadata's
        # thumbnail_filepath comment). Surface this as a normal, retryable download error
        # instead of letting an unhandled changeset bubble up and crash the job outright.
        message = "Failed to save media item after download: #{changeset_error_message(changeset)}"
  ...
  defp download_with_options(url, item_with_preloads, output_filepath, override_opts) do
    ...
    line_handler = fn line -> DownloadProgress.handle_line(item_with_preloads.id, line) end
    runner_opts = [output_filepath: output_filepath, use_cookies: should_use_cookies, line_handler: line_handler]
  ...
  # `changeset` may contain nested cast_assoc errors (eg: from the :metadata association),
  # so this walks the traversed error map recursively rather than assuming every value is
  ...
  defp flatten_error_map(error_map) do
    Enum.flat_map(error_map, fn
      {field, errors} when is_list(errors) ->
        Enum.flat_map(errors, fn
          error when is_binary(error) -> ["#{field}: #{error}"]
          nested_map when is_map(nested_map) -> Enum.map(flatten_error_map(nested_map), &"#{field}.#{&1}")
        end)
    end)
  end
end
