defmodule Vdlarr.Downloading.MediaDownloadWorker do
  @moduledoc false
  ...
  defp download_media_and_schedule_jobs(media_item, is_quality_upgrade, should_force) do
    ...
    case MediaDownloader.download_for_media_item(media_item, override_opts) do
      {:ok, downloaded_media_item} ->
        ...
        JellyfinNotificationWorker.kickoff(updated_media_item)
  ...
  @doc """
  Returns true if the given error message matches one of the known-unrecoverable
  yt-dlp errors (eg: the video was taken down). Used both to decide whether to
  automatically retry a failed download and to label such items in the Failed
  Downloads UI, so there's a single source of truth for the list.
  """
  def non_retryable_error?(message) when is_binary(message) do
    String.contains?(message, @non_retryable_errors)
  end

  def non_retryable_error?(_message), do: false
  ...
  defp action_on_error(message) do
    ...
    if non_retryable_error?(to_string(message)) do
