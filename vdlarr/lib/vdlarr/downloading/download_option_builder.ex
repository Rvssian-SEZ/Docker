defmodule Vdlarr.Downloading.DownloadOptionBuilder do
  @moduledoc """
  ...
  alias Vdlarr.Downloading.OutputPathBuilder
  alias Vdlarr.Downloading.QualityOptionBuilder
  ...
  defp default_options(override_opts) do
    ...
    [
      # `:progress` is required even though yt-dlp shows a progress bar by default -
      # `CommandRunner.global_options/0` passes `:quiet` to every call, which otherwise
      # suppresses `progress_template` output entirely. `:newline` keeps each update on
      # its own line (instead of a carriage-return-updating single line) so it can be
      # parsed line-by-line - see `Vdlarr.Downloading.DownloadProgress`.
      :progress,
      :newline,
      overwrite_behaviour,
      progress_template: "download:PROGRESS_JSON:%(progress)j",
  ...
  defp config_file_options(media_item) do
    base_dir = Path.join(Application.get_env(:vdlarr, :extras_directory), "yt-dlp-configs")
