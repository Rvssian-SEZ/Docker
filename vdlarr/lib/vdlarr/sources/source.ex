defmodule Vdlarr.Sources.Source do
  @moduledoc """
  The Source schema.
  """
  ...
  import Vdlarr.Utils.ChangesetUtils
  ...
  schema "sources" do
    ...
    # When set, overrides index_frequency_minutes - indexing is scheduled based
    # on this cron expression instead of a simple repeating interval. See
  ...
  @doc """
  Returns true if the source has a cron schedule set, meaning indexing is
  scheduled based on that expression instead of `index_frequency_minutes`.
  """
  def cron_scheduled?(%Source{index_cron_schedule: cron}), do: is_binary(cron)
  ...
  # `:video` sources are, by definition, a single video URL - so the regex that
  # blocks direct video URLs (meant to enforce channel/playlist-only sources) doesn't apply here.
  defp maybe_validate_url_format(changeset) do
    if get_field(changeset, :collection_type) == :video do
      changeset
    else
      validate_format(changeset, :original_url, youtube_channel_or_playlist_regex(), message: "must be a channel or playlist URL")
    end
  end
  ...
  defp validate_cron_schedule(%{changes: %{index_cron_schedule: cron}} = changeset) when is_binary(cron) do
    case Vdlarr.Utils.CronUtils.parse(cron) do
      {:ok, _} -> changeset
      {:error, reason} -> add_error(changeset, :index_cron_schedule, reason)
    end
  end

  defp validate_cron_schedule(changeset), do: changeset
