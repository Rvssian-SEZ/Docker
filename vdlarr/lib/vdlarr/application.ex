defmodule Vdlarr.Application do
  ...
  def start(_type, _args) do
    ...
    [
      ...
      {Phoenix.PubSub, name: Vdlarr.PubSub},
      # Start the Finch HTTP client for sending emails
      {Finch, name: Vdlarr.Finch},
      # Start a worker by calling: Vdlarr.Worker.start_link(arg)
      # {Vdlarr.Worker, arg},
      # Start to serve requests, typically the last entry (except for the post-boot tasks)
      VdlarrWeb.Endpoint,
      Vdlarr.Boot.PostBootStartupTasks
    ]
    |> Supervisor.start_link(strategy: :one_for_one, name: Vdlarr.Supervisor)
  end
  ...
  def config_change(changed, _new, removed) do
    VdlarrWeb.Endpoint.config_change(changed, removed)
    :ok
  end
  ...
  defp attach_oban_telemetry do
    ...
    :telemetry.attach_many("job-telemetry-broadcast", events, &VdlarrWeb.Telemetry.job_state_change_broadcast/4, [])
  end
  ...
  defp check_and_update_timezone do
    ...
    Application.put_env(:vdlarr, :timezone, valid_timezone)
  end
end
