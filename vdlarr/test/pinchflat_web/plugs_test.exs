defmodule PinchflatWeb.PlugsTest do
  ...
  describe "authenticate/2" do
    setup do
      old_basic_username = Application.get_env(:pinchflat, :basic_auth_username)
      old_basic_password = Application.get_env(:pinchflat, :basic_auth_password)
      old_login_username = Application.get_env(:pinchflat, :login_username)
      old_login_password = Application.get_env(:pinchflat, :login_password)

      on_exit(fn ->
        Application.put_env(:pinchflat, :basic_auth_username, old_basic_username)
        Application.put_env(:pinchflat, :basic_auth_password, old_basic_password)
        Application.put_env(:pinchflat, :login_username, old_login_username)
        Application.put_env(:pinchflat, :login_password, old_login_password)
      end)
    ...
    test "passes through when nothing is configured", %{conn: conn} do
      Application.put_env(:pinchflat, :basic_auth_username, nil)
      Application.put_env(:pinchflat, :basic_auth_password, nil)
      Application.put_env(:pinchflat, :login_username, nil)
      Application.put_env(:pinchflat, :login_password, nil)
    ...
    test "redirects to /login when session login is configured and there's no session", %{conn: conn} do
      Application.put_env(:pinchflat, :login_username, "user")
      Application.put_env(:pinchflat, :login_password, "pass")
