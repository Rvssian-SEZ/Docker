defmodule Pinchflat.Downloading.MediaDownloadWorkerTest do
  ...
  describe "perform/1 when testing user script callbacks" do
    ...
    test "enqueues a jellyfin notification job", %{media_item: media_item} do
      assert :ok = perform_job(MediaDownloadWorker, %{id: media_item.id})
  ...
  describe "non_retryable_error?/1" do
    test "returns true for known-unrecoverable yt-dlp errors" do
      ...
      assert MediaDownloadWorker.non_retryable_error?("Sign in to confirm your age")

      assert MediaDownloadWorker.non_retryable_error?(
               "This video is available to this channel's members"
             )
    end

    test "returns false for other errors" do
      refute MediaDownloadWorker.non_retryable_error?("Some transient network error")
    end

    test "returns false for nil or non-binary input" do
      refute MediaDownloadWorker.non_retryable_error?(nil)
    end
  end
end
