defmodule Vdlarr.SlowIndexing.SlowIndexingHelpersTest do
  use Vdlarr.DataCase
  ...
  describe "kickoff_indexing_task/3" do
    ...
    test "schedules based on the cron expression when the source has a cron schedule set, ignoring index_frequency_minutes" do
      source =
        source_fixture(
          index_frequency_minutes: 30,
          index_cron_schedule: "0 3 * * *",
          last_indexed_at: now_minus(5, :minutes)
        )
    ...
    test "cron scheduling is used even if the source has never been indexed" do
      source = source_fixture(index_cron_schedule: "0 3 * * *", last_indexed_at: nil)
  ...
  describe "scheduling_opts_for/1" do
    test "returns schedule_in based on index_frequency_minutes when no cron schedule is set" do
      source = source_fixture(index_frequency_minutes: 45, index_cron_schedule: nil)

      assert SlowIndexingHelpers.scheduling_opts_for(source) == [schedule_in: 45 * 60]
    end

    test "returns scheduled_at based on the cron expression when a cron schedule is set" do
      source = source_fixture(index_frequency_minutes: 45, index_cron_schedule: "0 3 * * *")

      assert [scheduled_at: %DateTime{} = scheduled_at] = SlowIndexingHelpers.scheduling_opts_for(source)
      assert scheduled_at.time_zone == "Etc/UTC"
      assert scheduled_at.hour == 3
  ...
  describe "index_and_enqueue_download_for_media_items/2" do
    ...
    test "doesn't blow up if the yt-dlp command itself fails", %{source: source} do
      stub(YtDlpRunnerMock, :run, fn _url, :get_media_attributes_for_collection, _opts, _ot, _addl_opts ->
        {:error, "ERROR: Error -3 while decompressing data: incorrect header check\n", 1}
      end)

      assert [{:error, _output, 1}] = SlowIndexingHelpers.index_and_enqueue_download_for_media_items(source)
    end

    test "still updates last_indexed_at and enqueues pending downloads if the yt-dlp command fails" do
      source = source_fixture()
      media_item = media_item_fixture(source_id: source.id, media_filepath: nil)
      ...
      assert DateTime.diff(DateTime.utc_now(), Repo.reload!(source).last_indexed_at) < 2
      assert_enqueued(worker: MediaDownloadWorker, args: %{"id" => media_item.id})
    end
  end
  ...
  describe "index_and_enqueue_download_for_media_items/2 when testing file watcher" do
    test "creates a new media item for everything already in the file", %{source: source} do
      watcher_poll_interval = Application.get_env(:vdlarr, :file_watcher_poll_interval)
