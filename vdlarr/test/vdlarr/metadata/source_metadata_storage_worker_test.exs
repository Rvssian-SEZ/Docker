defmodule Vdlarr.Metadata.SourceMetadataStorageWorkerTest do
  use Vdlarr.DataCase
  ...
  describe "perform/1 when testing attribute updates" do
    test "the source description is saved" do
      ...
      assert source.description == "This is a test file for Vdlarr"
    end
  end
    ...
    test "downloads and stores source images" do
      stub(YtDlpRunnerMock, :run, fn
        _url, :get_source_details, _opts, _ot, _addl ->
          filename = Path.join([Application.get_env(:vdlarr, :media_directory), "Season 1", "bar.mp4"])
