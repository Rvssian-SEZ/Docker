defmodule Vdlarr.SourcesTest do
  use Vdlarr.DataCase

  import Vdlarr.TasksFixtures
  ...
  describe "Source.cron_scheduled?/1" do
    test "returns true when the source has a cron schedule set" do
      source = source_fixture(%{index_cron_schedule: "0 3 * * *"})

      assert Source.cron_scheduled?(source)
    end

    test "returns false when the source does not have a cron schedule set" do
      source = source_fixture(%{index_cron_schedule: nil})

      refute Source.cron_scheduled?(source)
    end
  end
  ...
  describe "update_source/3 when testing cron scheduling" do
    test "adding a cron schedule to an existing source re-schedules the indexing task, even without touching index_frequency_minutes" do
      source = source_fixture(index_frequency_minutes: 60)
      update_attrs = %{index_cron_schedule: "0 3 * * *"}
    ...
    test "adding a cron schedule replaces the previously pending interval-based job, rather than adding a second one" do
      source = source_fixture(index_frequency_minutes: 60)
      {:ok, old_job} = Oban.insert(MediaCollectionIndexingWorker.new(%{"id" => source.id}))
      _old_task = task_fixture(source_id: source.id, job_id: old_job.id)

      update_attrs = %{index_cron_schedule: "0 3 * * *"}
      assert {:ok, %Source{}} = Sources.update_source(source, update_attrs)

      assert [new_job] = all_enqueued(worker: MediaCollectionIndexingWorker, args: %{"id" => source.id})
      assert new_job.id != old_job.id
    end

    test "removing a cron schedule reverts to interval-based scheduling" do
      source =
        source_fixture(index_frequency_minutes: 45, index_cron_schedule: "0 3 * * *", last_indexed_at: now())

      update_attrs = %{index_cron_schedule: nil}

      assert {:ok, %Source{} = source} = Sources.update_source(source, update_attrs)

      [job] = all_enqueued(worker: MediaCollectionIndexingWorker, args: %{"id" => source.id})
      assert_in_delta DateTime.diff(job.scheduled_at, DateTime.utc_now(), :minute), 45, 1
    end

    test "clearing a cron schedule with a blank string reverts to interval-based scheduling" do
      source =
        source_fixture(index_frequency_minutes: 45, index_cron_schedule: "0 3 * * *", last_indexed_at: now())

      update_attrs = %{index_cron_schedule: ""}

      assert {:ok, %Source{} = source} = Sources.update_source(source, update_attrs)
      assert source.index_cron_schedule == nil

      [job] = all_enqueued(worker: MediaCollectionIndexingWorker, args: %{"id" => source.id})
      assert_in_delta DateTime.diff(job.scheduled_at, DateTime.utc_now(), :minute), 45, 1
    end

    test "a disabled source does not get an indexing task even if a cron schedule is added" do
      source = source_fixture(enabled: false, index_frequency_minutes: 60)
      update_attrs = %{index_cron_schedule: "0 3 * * *"}
  ...
  describe "update_source/3 when testing options" do
    test "run_post_commit_tasks: false won't enqueue post-commit tasks" do
      source = source_fixture(%{download_media: false, index_frequency_minutes: -1})
      update_attrs = %{download_media: true, index_frequency_minutes: 100}
  ...
  describe "change_source/3 when testing cron schedule validation" do
    test "succeeds when a valid cron expression is provided" do
      source = source_fixture()

      assert %{errors: []} = Sources.change_source(source, %{index_cron_schedule: "0 3 * * *"})
    end

    test "succeeds when a cron schedule is set back to nil" do
      source = source_fixture(%{index_cron_schedule: "0 3 * * *"})

      assert %{errors: []} = Sources.change_source(source, %{index_cron_schedule: nil})
    end

    test "treats a blank string as nil" do
      source = source_fixture(%{index_cron_schedule: "0 3 * * *"})

      changeset = Sources.change_source(source, %{index_cron_schedule: "  "})

      assert %{errors: []} = changeset
      assert Ecto.Changeset.get_field(changeset, :index_cron_schedule) == nil
    end

    test "fails when an invalid cron expression is provided" do
      source = source_fixture()

      changeset = Sources.change_source(source, %{index_cron_schedule: "not a cron expression"})

      assert [_] = errors_on(changeset).index_cron_schedule
    end
  end
