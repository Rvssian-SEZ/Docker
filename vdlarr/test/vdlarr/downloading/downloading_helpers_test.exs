defmodule Vdlarr.Downloading.DownloadingHelpersTest do
  use Vdlarr.DataCase
  ...
  describe "retry_download/2" do
    test "enqueues a forced download job regardless of pending-eligibility" do
      media_item = media_item_fixture(media_filepath: nil, last_error: "Some error")

      assert {:ok, _} = DownloadingHelpers.retry_download(media_item)
    ...
    test "can pass job options" do
      ...
      assert {:ok, _} = DownloadingHelpers.retry_download(media_item, priority: 1)

      assert_enqueued(worker: MediaDownloadWorker, args: %{"id" => media_item.id}, priority: 1)
    end
  end

  describe "enqueue_failed_download_tasks/2" do
    test "enqueues a forced download job for each failed media item" do
      source = source_fixture()
      media_item = media_item_fixture(source_id: source.id, media_filepath: nil, last_error: "Some error")

      assert :ok = DownloadingHelpers.enqueue_failed_download_tasks(source)

      assert_enqueued(worker: MediaDownloadWorker, args: %{"id" => media_item.id, "force" => true})
    end

    test "does not enqueue a job for media items without an error" do
      source = source_fixture()
      _media_item = media_item_fixture(source_id: source.id, media_filepath: nil, last_error: nil)
    ...
    test "does not enqueue a job for a media item that both downloaded and has a stale error" do
      source = source_fixture()

      _media_item =
        media_item_fixture(source_id: source.id, media_filepath: "some/filepath.mp4", last_error: "Some error")
    ...
    test "does not enqueue a job for a manually ignored media item" do
      source = source_fixture()
    ...
    test "does not enqueue a job if the source is set to not download" do
      source = source_fixture(download_media: false)
