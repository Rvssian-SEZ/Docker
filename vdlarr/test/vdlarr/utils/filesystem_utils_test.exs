defmodule Vdlarr.Utils.FilesystemUtilsTest do
  use Vdlarr.DataCase
  ...
  describe "list_media_subdirectories/0" do
    setup do
      original_media_directory = Application.get_env(:vdlarr, :media_directory)
      media_directory = Path.join(tmpfile_directory(), "media_subdirs_test")
    ...
    test "lists top-level subdirectories", %{media_directory: media_directory} do
      File.mkdir_p!(Path.join(media_directory, "Channel A"))
      File.mkdir_p!(Path.join(media_directory, "Channel B"))

      assert FilesystemUtils.list_media_subdirectories() == ["Channel A", "Channel B"]
    end

    test "returns entries sorted alphabetically", %{media_directory: media_directory} do
      File.mkdir_p!(Path.join(media_directory, "Zebra"))
      File.mkdir_p!(Path.join(media_directory, "Apple"))

      assert FilesystemUtils.list_media_subdirectories() == ["Apple", "Zebra"]
    end

    test "excludes files, only listing directories", %{media_directory: media_directory} do
      File.mkdir_p!(Path.join(media_directory, "Channel A"))
      FilesystemUtils.write_p!(Path.join(media_directory, "not_a_directory.txt"), "")

      assert FilesystemUtils.list_media_subdirectories() == ["Channel A"]
    end

    test "excludes dotfiles/dotdirectories", %{media_directory: media_directory} do
      File.mkdir_p!(Path.join(media_directory, "Channel A"))
      File.mkdir_p!(Path.join(media_directory, ".hidden"))

      assert FilesystemUtils.list_media_subdirectories() == ["Channel A"]
    end

    test "returns an empty list if the media directory doesn't exist" do
      Application.put_env(:vdlarr, :media_directory, "/nonexistent/media/directory")

      assert FilesystemUtils.list_media_subdirectories() == []
    end
  end

  defp tmpfile_directory do
    Application.get_env(:vdlarr, :tmpfile_directory)
  end
end
