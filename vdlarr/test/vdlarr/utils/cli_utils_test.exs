defmodule Vdlarr.Utils.CliUtilsTest do
  use Vdlarr.DataCase
  ...
  describe "wrap_cmd/3" do
    ...
    test "with a line_handler, calls it with each line of output" do
      test_pid = self()
      handler = fn line -> send(test_pid, {:line, line}) end

      CliUtils.wrap_cmd("printf", ["line1\\nline2\\n"], [], line_handler: handler)

      assert_received {:line, "line1"}
      assert_received {:line, "line2"}
    end

    test "with a line_handler, still returns the full output as a binary" do
      handler = fn _line -> :ok end

      assert {output, 0} = CliUtils.wrap_cmd("printf", ["line1\\nline2\\n"], [], line_handler: handler)
      assert output == "line1\nline2"
    end
  end
