defmodule Vdlarr.YtDlp.MediaCollectionTest do
  use Vdlarr.DataCase

  import Vdlarr.SourcesFixtures

  alias Vdlarr.YtDlp.Media
  alias Vdlarr.YtDlp.MediaCollection

  @channel_url "https://www.youtube.com/c/VdlarrTestChannel"
    ...
    test "returns a map with data on success" do
      expect(YtDlpRunnerMock, :run, fn _url, :get_source_details, _opts, _ot, _addl_opts ->
        Phoenix.json_library().encode(%{
          channel: "VdlarrTestChannel",
          channel_id: "UCQH2",
          playlist_id: "PLQH2",
          playlist_title: "VdlarrTestChannel - Videos"
        })
      end)
      ...
      assert %{
               channel_id: "UCQH2",
               channel_name: "VdlarrTestChannel",
               playlist_id: "PLQH2",
               playlist_name: "VdlarrTestChannel - Videos"
             } = res
    end
  ...
  describe "get_source_metadata/1" do
    test "returns a map with data on success" do
      expect(YtDlpRunnerMock, :run, fn _url, :get_source_metadata, _opts, _ot, _addl_opts ->
        Phoenix.json_library().encode(%{channel: "VdlarrTestChannel"})
      end)

      assert {:ok, res} = MediaCollection.get_source_metadata(@channel_url, playlist_items: 0)

      assert %{"channel" => "VdlarrTestChannel"} = res
    end
