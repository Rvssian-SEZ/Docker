defmodule CofferWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: CofferWeb.Gettext
  ...
  places, e.g. `1000000` -> `"1,000,000.00"`. Display only — never apply
  this to a value going into an `<.input>` (breaks HTML number parsing) or
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(CofferWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(CofferWeb.Gettext, "errors", msg, opts)
    end
  end
